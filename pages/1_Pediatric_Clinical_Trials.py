...
def main():
    ...
    # REMOVE THE WITH STATEMENT - engine is not a context manager
    ...
    if safe_get_session_state('pediatric_trials') is None or safe_get_session_state('need_data_reload', True):
        with st.spinner("Loading pediatric trials from the AACT database..."):
            ...
            pediatric_trials = fetch_pediatric_trials_in_canada(engine)
            ...
            if not pediatric_trials.empty:
                ...
                facilities_df = fetch_facilities_for_trials(engine)
    ...
    try:
        # Use text() to convert the SQL string to a SQLAlchemy TextClause object
    ...
    if pediatric_df is not None and not pediatric_df.empty:
        ...
        if filtered_df.empty:
            ...
            with tab3:
                # Use engine instead of conn
                render_city_visualization(filtered_df, engine)  # Change conn to engine
                render_city_visualization_normalized(filtered_df, engine)  # Change conn to engine
                render_province_visualization(filtered_df, engine, title_prefix="Pediatric")  # Change conn to engine

            with tab4:
                # Add interventions and conditions analysis
                render_interventions_conditions_analysis(
                    filtered_df, 
                    engine,  # Change conn to engine 
                    title_prefix="Pediatric",
                    conditions_dict=safe_get_session_state('conditions_dict', {}),
                    interventions_df=fetch_interventions_for_trials(engine, filtered_df['nct_id'].tolist())  # Change conn to engine
                )
